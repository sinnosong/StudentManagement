...
namespace StudentManagement.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            if (role == null)
            {
                ViewBag.ErrorMessage = $"无法找到角色id为{id}的角色不存在";
                return View("NotFound");
            }
            else
            {
                ...
                catch (DbUpdateException ex)
                {
                    ...
                    ViewBag.ErrorMessage = $"无法删除{role.Name}角色，因为此角色中已经存在用户。如果想删除此角色，需要先将角色名下的用户删除，再删除角色本身";
                    return View("Error");
                }
            }
        }
        ...
        public async Task<IActionResult> EditUser(EditUserViewModel model)
        {
            ...
            if (user == null)
            {
                ViewBag.Title = $"无法找到Id为{model.Id}的用户";
                return View("NotFound");
            }
            else
            {
        ...
        [HttpPost]
        public async Task<IActionResult> ManageUserRoles(List<RolesInUserViewModel> models, string userId)
        {
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null)
            {
            ...
            result = await _userManager.AddToRolesAsync(user, models.Where(u => u.IsSelected).Select(y => y.RoleName));
            if (!result.Succeeded)
            {
        ...
        public async Task<IActionResult> ManageUserClaims(UserClaimsViewModel model)
        {
            ...
            if (!result.Succeeded)
            {
                ModelState.AddModelError("", "无法向当前用户添加声明");
                return View(model);
            }
